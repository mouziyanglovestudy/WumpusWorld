...
public class MyAI extends Agent
{
	 ...
	 */
	private Set<Cell> getNearbyCell(int cell) {
	    Set<Cell> nearbyCells = new HashSet<Cell>();
	    for (int i : FOUR_DIRECTIONS) {
	        int nearbyCellCoords = cell + i;
	        if (!(cell % 10 == 0 && nearbyCellCoords % 10 == 9) 
	                && !(cell % 10 == 9 && nearbyCellCoords % 10 == 0)) {
	            Cell nearbyCell = allCells.get(nearbyCellCoords);
	            if (nearbyCell != null) {
	...
	private void checkBreeze(boolean isBreeze) {
	    if (!isBreeze) {
	        for (Cell cell : getNearbyCell(currentCell)) {
	...
	private void checkStench(boolean isStench) {
	    if (!isStench) {
	        for (Cell cell : getNearbyCell(currentCell)) {
	            ...
	            wumpusCells.remove(cell.getIntCoord());
	        }
	    }
	    else {
	        ...
	        for (Cell cell: getNearbyCell(currentCell)) {
                if (cell.mightWumpus()) {
                    wumpusCells.add(cell.getIntCoord());
	            }
	        }
	    }
	}
	...
	private void addToStack() {
	    for (int i: new int[]{getBehindDirection(direction), getLeftDirection(direction), 
	        ...
	        if (cell != null && !(currentCell % 10 == 0 && i == -1) && !(currentCell % 10 == 9 && i == 1)) {
	...
	private Action moveToGoal(int goal) {
	    ...
	    for (int i : FOUR_DIRECTIONS) {
	        int cellCoords = currentCell + i;
	        Cell cell = allCells.get(cellCoords);
	        if (cell != null && !cell.mightPit() && !cell.mightWumpus() 
	                && !(currentCell % 10 == 0 && cellCoords % 10 == 9) 
	                && !(currentCell % 10 == 9 && cellCoords % 10 == 0)) {
                tempList.add(cell);
            }
	    }
	...
	private void killWumpus() {
	    int wumpusCellCoords = (int) wumpusCells.toArray()[0];
	    /*for (int cellCoords : getNearbyCellCoords(wumpusCellCoords)) {
	        Cell cell = allCells.get(cellCoords);
	    }*/
	}
	// ======================================================================
